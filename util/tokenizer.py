...
class Tokenizer(DiceRoller):
    ...
    @classmethod
    def __are_tokenized(cls, items):
        for s in items:
            if cls.__has_token(s):
                return False
        return True
    ...
    def get_tokens(cls, exp):
        items = exp.split()
        while not cls.__are_tokenized(items):
